...
LabelsPtr TTSManager::AnalyzeTextFragment(const TextFragmentPtr& tf) {
   ...
   __android_log_print(ANDROID_LOG_DEBUG, "TTSManager", "AnalyzeTextFragment: Analyzing Text.");
   TextAnalyzerPtr textAnalyzer = GetTextAnalyzer(tf->GetProperties());
   LOG_DEBUG("[TTSManager::AnalyzeTextFragment] Call Analyzer.");
   __android_log_print(ANDROID_LOG_DEBUG, "TTSManager", "AnalyzeTextFragment: Call Analyzer.");
...
TextAnalyzerPtr TTSManager::GetTextAnalyzer(const FragmentPropertiesPtr props) {
   __android_log_print(ANDROID_LOG_DEBUG, "TTSManager", "GetTextAnalyzer: in function first line");
   ...
   if (requestedAnalyzer == PROPERTY_VALUE_AUTOMATIC) {
      // language given?
      __android_log_print(ANDROID_LOG_DEBUG, "TTSManager", "GetTextAnalyzer: language given?");
      if (props->find(PROPERTY_KEY_LANGUAGE) !=  props->end()) {
         // german? use internal
         if ((*props)[ PROPERTY_KEY_LANGUAGE ].substr(0, 2) == "de") {
            __android_log_print(ANDROID_LOG_DEBUG, "TTSManager", "Setting synthesis language de setting");
            requestedAnalyzer = PROPERTY_VALUE_INTERNAL;
         }
         // english? use flite (if available)
         else if ((*props)[ PROPERTY_KEY_LANGUAGE ].substr(0, 2) == "en") {
            __android_log_print(ANDROID_LOG_DEBUG, "TTSManager", "Setting synthesis language en setting");
            requestedAnalyzer = PROPERTY_VALUE_FLITE;
         }
         // albanian? use internal
         else if ((*props)[ PROPERTY_KEY_LANGUAGE ].substr(0, 2) == "sq") {
             __android_log_print(ANDROID_LOG_DEBUG, "TTSManager", "Setting synthesis language sq setting");
             requestedAnalyzer = PROPERTY_VALUE_INTERNAL;
         }
      }
   }
